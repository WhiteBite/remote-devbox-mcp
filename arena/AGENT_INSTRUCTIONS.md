...
3. `./mcp check` — рукопожатие и список тулов. Есть ответ — мост жив, работаю.
   ...
   530 — норма (туннель переподключается), персистентные — туннель лежит,
   прошу пользователя проверить `docker compose logs cloudflared` и линию/VPN.
...
- Тулчейны в образе зависят от build-аргументов: `WITH_JAVA=1` → JDK 21,
  `WITH_FLUTTER=1` → Flutter/Dart SDK. Перед сборкой проверяю через `bash`:
  `java -version`, `flutter --version`. Чего не хватает — либо ставлю сам в
  `/opt/tools` (персистентный volume, `/opt/tools/bin` уже в PATH), либо
  прошу пользователя пересобрать образ с нужным аргументом.
...
Работают, если стек испечён в образ (`WITH_JAVA`/`WITH_FLUTTER`) или установлен
в `/opt/tools`.
